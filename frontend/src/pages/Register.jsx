...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!hasSpecialChar) {
            setValidationError("Password must contain at least one special symbol (@#$%).");
            return;
        }
    ...
    return (
        <div className="min-h-screen w-full bg-[#070709] flex items-center justify-center p-4 md:p-8 font-sans overflow-y-auto">
            ...
            <div className="w-full max-w-5xl bg-[#0e0e12]/80 border border-zinc-800/80 rounded-[32px] overflow-hidden grid grid-cols-1 md:grid-cols-2 p-3 gap-8 min-h-[600px] shadow-[0_24px_50px_-12px_rgba(0,0,0,0.7)] backdrop-blur-xl relative z-10">
                ...
                <div className="flex flex-col justify-center px-4 py-8 md:p-10">
                    <div className="w-full max-w-md mx-auto">
                        <div className="text-center md:text-left mb-6">
                            <h3 className="text-3xl font-bold text-white tracking-tight">Sign Up An Account</h3>
                            <p className="text-zinc-400 text-sm mt-2">Enter personal data to create your account</p>
                        </div>

                        {/* Registration Form */}
                        <form onSubmit={handleSubmit} className="space-y-4">
                            {/* Side-by-side username & email on medium+ screens */}
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <button
                                        ...
                                        className="absolute scale-85 right-3 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-zinc-300 transition-colors"
                                    >
                                        {showPassword ? (
                                ...
                                <div className="mt-3 space-y-1.5">
                                    <div className="flex items-center gap-2 text-xs">
                                        <div className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center transition-all ${hasSpecialChar ? 'bg-violet-600/20 border-violet-500 text-violet-400' : 'border-zinc-800 text-transparent'}`}>
                                            <svg className="w-2.5 h-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                                                <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                                            </svg>
                                        </div>
                                        <span className={hasSpecialChar ? 'text-violet-400' : 'text-zinc-500'}>
                                            Key must have special symbols like @#$% etc.
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-2 text-xs">
                                        <div className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center transition-all ${isLongEnough ? 'bg-violet-600/20 border-violet-500 text-violet-400' : 'border-zinc-800 text-transparent'}`}>
                                            <svg className="w-2.5 h-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                                                <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                                            </svg>
                                        </div>
                                        <span className={isLongEnough ? 'text-violet-400' : 'text-zinc-500'}>
                                            Key must be longer than 8 symbols
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                <label htmlFor="confirmPassword" className="block text-xs font-semibold text-zinc-400 mb-1.5 uppercase tracking-wider">
                                    Confirm Password
                                </label>
                                <div className="relative">
                                    <input
                ...
                <div style={{ backgroundImage: `url(${bg})`, backgroundSize: 'cover', backgroundPosition: 'center' }} className="hidden md:flex flex-col justify-between p-10 rounded-[24px] relative overflow-hidden border border-zinc-800/30 min-h-[500px]">
                    {/* Logo */}
